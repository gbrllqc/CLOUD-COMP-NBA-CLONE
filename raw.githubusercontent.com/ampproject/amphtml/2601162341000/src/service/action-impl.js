        ...
        do {
            ...
            if (type == TokenType_Enum.SEPARATOR && (value == ',' || value == ')')) {
                ...
                if (tok.type == TokenType_Enum.ID) {
                    for (
                        peek = toks.peek(); peek.type == TokenType_Enum.SEPARATOR && peek.value == '.'; peek = toks.peek()
                    ) {
                        toks.next(); // Skip '.'.
...
function argValueForTokens(tokens) {
    if (tokens.length == 0) {
        ...
        return /** @type {ActionInfoArgExpressionDef} */ ({
            expression
        });
    }
}
